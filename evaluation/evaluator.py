...
def evaluate(df_groundtruth, df_parsedlog, filter_templates=None):
    ...
    (GA, FGA) = get_accuracy(
        df_groundtruth['EventTemplate'], df_parsedlog['EventTemplate'], filter_templates)
...
def get_accuracy(series_groundtruth, series_parsedlog, filter_templates=None):
    ...
    for ground_truthId, group in tqdm(grouped_df):
        ...
        #     else:
        #         print(f"{ground_truthId}_{len(group)}")
        #         series_parsedlog[series_parsedlog ==
        #                          parsed_eventId].to_csv(f'error_{len(group)}_{series_parsedlog[series_parsedlog == parsed_eventId].size}.csv')
        # else:
        #     print(f"{series_parsedlog_logId_valuecounts}")
        #     group['parsedlog'].to_csv(
        #         f'error_{series_parsedlog_logId_valuecounts.size}.csv')
